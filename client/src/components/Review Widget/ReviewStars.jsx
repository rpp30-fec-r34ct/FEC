...
const ReviewStars = (props) => {
  ...
  for (let i = 0; i < wholeStars.length; i++) {
    wholeStars[i] = <img alt="whole-review-stars" key={props.review_id + i} src={fullStar} />
  }

  if (decimalRating > 0 && decimalRating <= 0.3) {
    decimalStars.push(<img alt="decimal-review-stars" key={props.review_id + decimalStars.length - 1} src={quarterStar} />)
  } else if (decimalRating > 0.3 && decimalRating <= 0.6) {
    decimalStars.push(<img alt="decimal-review-stars" key={props.review_id + decimalStars.length - 1} src={halfStar} />)
  } else if (decimalRating > 0.6 && decimalRating <= 0.9) {
    decimalStars.push(<img alt="decimal-review-stars" key={props.review_id + decimalStars.length - 1} src={threeQuarterStar} />)
  }

  starArray = wholeStars.concat(decimalStars)
  if (starArray.length < 5) {
    for (let i = starArray.length; i < 5; i++) {
      starArray.push(<img alt="whole-review-stars" key={props.review_id + i} src={emptyStar} />)
    }
  }
